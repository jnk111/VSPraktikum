...
public final class ExceptionUtils {
	 ...
	 */
	public static String getExceptionInfo(final Throwable e, String lvl) {
		...
		return String.format(mask, lvl, e.getClass().getCanonicalName(), msg, ExceptionUtils.getSimplifiedTrace(e));
	}
	 ...
	 */
	public static String getSimplifiedTrace(final Throwable e) {
		// load stack trace
		...
		for (int i = 0; i < stackTrace.length; i++) {
			...
			if (skip) {
				...
				if (skipped > 0) {
					...
					trace.append(String.format(traceElement, i - 1, prev.getClassName(), prev.getMethodName(), prev.getFileName(),
							prev.getLineNumber()));
				}
				// print current element
				trace.append(String.format(traceElement, i, elem.getClassName(), elem.getMethodName(), elem.getFileName(),
						elem.getLineNumber()));
			}
			prev = elem;
		}
	 ...
	 * 
	 * @param filters
	 *          Package names used for filtering. All sub-packages of the given
