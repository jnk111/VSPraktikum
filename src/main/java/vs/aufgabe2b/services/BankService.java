...
public class BankService {
	...
	public BankService() {
		...
		Spark.get(BankService.URL_BANKS_BASE + ":id/", (request, response) -> {
			...
			response.type(BankService.JSON_MIME_TYPE);
			final BankData res = this.getBank(bankId);
			if (res == null) {
				response.status(404);
				return null;
			} else {
		...
		Spark.put(BankService.URL_BANKS_BASE + ":id/", (request, response) -> {
			...
			if (this.updateBank(bankId, data)) {
				response.status(200);
			} else {
				response.status(404);
			}
	 ...
	 * @return Bank data.
	 */
	private BankData createBank(final GameId game) {
	 ...
	 */
	private BankData getBank(final String id) {
	 ...
	 * @return <code>true</code> if the bank exists, <code>false</code> otherwise.
	 */
	private boolean updateBank(final String id, final BankData data) {
		final Bank bank = BankFactory.getBank(id);
		if (bank == null) {
			return false;
		}
