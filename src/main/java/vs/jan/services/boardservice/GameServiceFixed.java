...
public class GameServiceFixed {
	...
	private final boolean LOCAL = true; // Zu Testzwecken: LOCAL auf true, wenn
																			// alle Services lokal laufen

	private static Map<String, Game> games;

	public GameServiceFixed() {
		games = new HashMap<>();
	...
	private void initPostNewGame() {
		post("/games", (req, resp) -> {
			...
			Game game = createGame(req.body()); // TODO Fehlerbehandlung: kein Game
																					// name angegeben
	...
	private Game initGameServices(Game newGame) {
		...
		if (!LOCAL) {
			try {
				...
				newGame.setPlayers(newServices.get("users")); // TODO: Userservice ( Da
																											// Userservice laut Spezi
																											// nicht unter Services
																											// gespeichert ist )
	 ...
	 * auf Userservice zu um Spieler zu speichern.
	 */
	private void initPostNewPlayer() {
		post("/games/:gameId/players", (req, resp) -> {
			...
			Player newPlayer = new Gson().fromJson(req.body(), Player.class); // Erstellt
																																				...
																																				// mit
																																				// Namen
			newPlayer.setId(newPlayer.getUserName().toLowerCase()); // TODO: ID so
																															// richtig? oder
																															// komplette URL?
			newPlayer.setUserName(newPlayer.getUserName().toLowerCase());
			if (game != null && !game.getPlayers().containsKey(newPlayer.getId())) {
				CreateUserDTO userDTO = new CreateUserDTO();
				userDTO.setId(newPlayer.getId());
				userDTO.setName(newPlayer.getUserName().toLowerCase());
