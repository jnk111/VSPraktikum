...
public class GamesService {
	...
	public synchronized String postNewGame(Request req, Response resp) {
		...
		if (game != null && game.isValid()) {
			if (!games.containsKey(game.getId().toLowerCase())) {
				...
				resp.status(GAME_ALREADY_EXCITS_RESP_CODE); // Conflict: Spiel mit
																										// selben Namen bereits
																										// vorhanden
			}
		} else {
			if (DEBUG_MODE)
	...
	private Game initGameServices(Game newGame) {
		Map<String, String> newServices = new HashMap<>(); // Map zum Speichern und
																												// anschliessendem
																												// Uebertragen in die
																												// ServiceList des Games

		if (!LOCAL) {
			try {
				HttpResponse<JsonNode> response = Unirest.get(YELLOW_PAGE + YP_GROUP_CMD + YP_GROUP_NAME).asJson(); // Service-Adressen
																																																						// von
																																																						// YP
																																																						// holen
				ServiceArray servicesOfGroup = new Gson().fromJson(response.getBody().toString(), ServiceArray.class); // Service-Adressen
																																																								// in
																																																								// Array
																																																								// schreiben
				...
				newGame.getServiceList().setAllServices(newServices); // Services in
																															// neuem Spiel
																															// speichern
	...
	private Game createBoard(Game game) {
		...
		try {
			...
			System.err.println(
					"Prozess hat in der Wartephase zwischen Erstellen und Initialisieren eines Boards ein Interrupt empfangen.");
		}
		// ******************************************************************************
		// //

		boardsUrl = (boardsUrl + "/" + game.getName()); // Spiele-Namen an
																										// Boards-URL anfuegen
	...
	/**
	 * Initialisiert die PUT-Methode zum setzen von ein oder mehrerer Komponenten
	 * fuer ein Spiel.
	 * 
	...
	/**
	 * Initialisiert die Post-Methode zum erstellen eines neuen Spielers. Greift
	 * auf Userservice zu und Speichert den Spieler dort (kA wozu TODO).
	 * 
	 * return nur Statuscodes ( 201 falls Spieler erfolgreich erstellt wurde; 409
	 * falls Spieler bereits vorhanden )
	 * 
	...
	public synchronized String postNewPlayer(Request req, Response resp) {
		...
		if (!game.isRunning()) {
			...
			Player newPlayer = new Gson().fromJson(req.body(), Player.class); // Erstellt
																																				// Playerobjekt
																																				// mit
																																				// Namen
			String mapKey = newPlayer.getUserName().toLowerCase();

			// ================= Playerobjekt wird entsprechend der Spezi fuer
			// GameService konfiguriert ================= //
			...
			if (game != null && !game.getPlayers().containsKey(mapKey)) {
				...
				// ================= Post an UserService (User wird im UserService
				// erstellt) ================= //
	...
	private Player createPawn(Player player, Game game) {
		...
		// newPawn.setPlace( "/boards/" + game.getName() + "/places/" + "0" ); //
		// TODO muss automatisiert werden, aber wie? Wer "putet", gameservice oder
		// boards?
		...
		if (responseCode == 200) {
			player.setPawn("/boards/" + game.getName() + "/pawns" + player.getUserName().replaceAll("/users", ""));

			if (DEBUG_MODE)
				System.out.println("New Pawn URI : " + player.getPawn());
		} else {
			System.err.println("Pawn erstellen fehlgeschlagen"); // TODO
																														// Fehlerbehandlung
																														// falls Pawn nicht
																														// erstellt werden
																														// kann (Neuversuch
	...
	private void createAccount(Player player, Game game) {
		...
		if (responseCode == 201) {
			...
			System.err.println("Account erstellen fehlgeschlagen"); // TODO
																															// Fehlerbehandlung
																															// falls Pawn
																															// nicht erstellt
																															// werden kann
	...
	public String putPlayerReadyness(Request req, Response resp) {
		...
		if (game != null) {
			...
			if (player != null) {
				...
				if (game.isRunning()) {
					if (!player.getId().equals(mutexService.getMutexUser(game.getId()))) // Falls
																																								// der
																																								// Spieler
																																								// momentan
																																								// nicht
																																								...
																																								// conflict
					{
						...
						initNextPlayersTurn(game); // Erlaubnis fuer das Inanspruchnehmen
																				// des Mutex an naechsten Spieler weiter
																				// geben
					}
	...
	private void initNextPlayersTurn(Game game) {
		...
		// neu
		// -------------------------------------------------------------------------
		Player nextPlayer = playerArray[nextPlayerIndex];
		JSONTurn playerTurn = new JSONTurn(nextPlayer.getId());
		// -------------------------------------------------------------------------
		int responseCode = HttpService.post(nextPlayer.getUri() + "/turn", playerTurn);
	...
	public String getSpecificPlayer(Request req, Response resp) {
		...
		if (player != null) {
			...
			specificPlayerDTO.setReady(
					game.getComponents().getGame() + "/players/" + player.getUserName().replaceAll("/users/", "") + "/ready");
	...
	/**
	 * Veraendert bei Aufruf, entsprechend der vorliegenden Daten, den Status des
	 * Spiels. Standardwert: "registration", nach Erstellen des Spiels. Das
	 * Aufrufen der put-Methode bewirkt erst eine Veraenderung, wenn der Status
	 * aller Spieler auf "ready" geaendert wurde, dann aendert sich bei Aufruf der
	 * Status auf "running". Sobald alle Kriterien erfuellt sind, die das Spiel
	 * laut Regeln beenden, bewirkt das erneute Aufrufen der put-Methode eine
	 * Aenderung des Statuses in "finished".
	 * 
	 * return nur Statuscodes ( 200: Status geaendert; 409: Status konnte nicht
	 * geaendert werden )
	 * 
	 * URI: /games/:gameId/status
	 */
	...
	/**
	 * Versucht den Mutex fuer den jeweiligen Spieler zuerhalten. Klappt nur wenn
	 * dieser auch an der Reihe ist.
	 * 
	 * Uebergeben werden muss ein Playerobjekt als json. ( Die Methode verwendet
	...
	//
	// if ( game != null && game.isRunning() && player != null ) // TODO
	// Fehlercode falls der Spieler den Mutex bereits hat
	// {
	...
	public String getPlayersTurn(Request req, Response resp) {
		...
		if (game != null && game.isRunning() && !mutexService.getMutexUser(game.getId()).equals("")) {
			...
			currentPlayerDTO.setUser(currentPlayer.getUserName()); // TODO soll die
																															// Uri sein
			currentPlayerDTO.setPawn(currentPlayer.getPawn());
			currentPlayerDTO.setAccount(currentPlayer.getAccount());
			currentPlayerDTO.setReady(game.getComponents().getGame() + "/players/"
					+ currentPlayerDTO.getUser().replaceAll("/user/", "") + "/ready");
